...
export class BannerListComponent {
    ...
    loadBLogs(event: any) {
        ...
        this.bannerService.getAll(queryParams).subscribe((data: any) => {
    ...
    confirm2(event: Event, user) {
        this.confirmationService.confirm({
            ...
            accept: () => {
                this.bannerService.delete(user._id).subscribe((res) => {
    ...
    filter() {
        ...
        let queryParams = this.commonService.getHttpParamsByJson(params);
        this.bannerService.getAll(queryParams).subscribe((res) => {
            this.appointments = res;
        });
    }
    async updateStatus(id, status) {
        this.bannerService.update(id, { status }).subscribe((res) => {
            this.messageService.add({
                // key: 'tst',
                severity: 'success',
